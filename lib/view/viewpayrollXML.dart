...
*/
    ...
    List<Widget> getList()
    {
      ...
      list.add(createRow4(screenWidth, 18, 'TĂNG CA CHỦ NHẬT 晚班加班+假日班 (270%)', Utility.numbertoStringFormat(fild02aController.getitem().TCD20!, "#,###.#"), Utility.numbertoStringFormat(fild02aController.getitem().TienTCD20!, "#,###"),false));
      list.add(createRow4(screenWidth, 19, 'TẾT 節日加班 (300%)', Utility.numbertoStringFormat(fild02aController.getitem().TC30!, "#,###.#"), Utility.numbertoStringFormat(fild02aController.getitem().TienTC30!, "#,###"),false));
      list.add(createRow4(screenWidth, 20, 'TĂNG CA ĐÊM LỄ 節日 (390%)', Utility.numbertoStringFormat(fild02aController.getitem().TCD30!, "#,###.#"), Utility.numbertoStringFormat(fild02aController.getitem().TienTCD30!, "#,###"),false));
      list.add(createRow4(screenWidth, 21, 'CƠM  伙食 (25.000/ 1 SUẤT ĂN 次)', Utility.numbertoStringFormat(fild02aController.getitem().SoPhanCom!, "#,###"), Utility.numbertoStringFormat(fild02aController.getitem().PCTienCom!, "#,###"),false));
      list.add(createRow4(screenWidth, 22, 'CƠM CÔNG TÁC 伙食  (35.000/ 1 SUẤT ĂN 次)', Utility.numbertoStringFormat(fild02aController.getitem().SoPhanComCT!, "#,###"), Utility.numbertoStringFormat(fild02aController.getitem().TienComCongTac!, "#,###"),false));

      list.add(createRow3(screenWidth, 23, 'THƯỞNG CHUYÊN CẦN 全勤獎金', Utility.numbertoStringFormat(fild02aController.getitem().ChuyenCan!, "#,###"), false));
      list.add(createRow3(screenWidth, 24, 'T/T PHÉP NĂM CL 特休補助', Utility.numbertoStringFormat(fild02aController.getitem().TienTraPhepNam!, "#,###"), false));
      list.add(createRow3(screenWidth, 25, 'PC NUÔI CON NHỎ 撫育小孩津貼', Utility.numbertoStringFormat(fild02aController.getitem().PCNuoiConCB!, "#,###"), false));
      list.add(createRow3(screenWidth, 26, 'PC KINH NGUYỆT 生理期補貼', Utility.numbertoStringFormat(fild02aController.getitem().PCPhuNu!, "#,###"), false));
      list.add(createRow3(screenWidth, 27, 'THU NHẬP KHÁC 其他補助', Utility.numbertoStringFormat(fild02aController.getitem().CongKhac!, "#,###"), false));

      list.add(createRow1(screenWidth, 'CÁC KHOẢN GIẢM TRỪ KHÁC 扣除額',false));
      list.add(createRow3(screenWidth, 28, 'VẮNG MẶT 請假', Utility.numbertoStringFormat(fild02aController.getitem().TruVangMat!, "#,###"), false));
      list.add(createRow3(screenWidth, 29, 'BHXH 社會保險 (8%)', Utility.numbertoStringFormat(fild02aController.getitem().BHXH!, "#,###"), false));
      list.add(createRow3(screenWidth, 30, 'BHYT 醫療保險 (1.5%)', Utility.numbertoStringFormat(fild02aController.getitem().BHYT!, "#,###"), false));
      list.add(createRow3(screenWidth, 31, 'BHTN 失業保險 (1%)', Utility.numbertoStringFormat(fild02aController.getitem().BHTN!, "#,###"), false));
      list.add(createRow3(screenWidth, 32, 'CÔNG ĐOÀN 工會 (1%)', Utility.numbertoStringFormat(fild02aController.getitem().CongDoan!, "#,###"), false));
      list.add(createRow3(screenWidth, 33, 'THUẾ TNCN 個人所得稅', Utility.numbertoStringFormat(fild02aController.getitem().tax_ThueTNCN!, "#,###"), false));
      list.add(createRow3(screenWidth, 34, 'GIẢM TRỪ KHÁC 其他', Utility.numbertoStringFormat(fild02aController.getitem().TruKhac!, "#,###"), true));
      list.add(createRow3Bold(screenWidth, 35, 'THỰC LÃNH 實領薪資 = (4)-(28+29+30+31+32+33+34)', Utility.numbertoStringFormat(fild02aController.getitem().ThucLanh!, "#,###"), true));
